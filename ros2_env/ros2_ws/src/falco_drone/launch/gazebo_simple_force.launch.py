...
def generate_launch_description():
    ...
    model_arg = DeclareLaunchArgument(
        name="model",
        default_value=os.path.join(pkg_share, "urdf", "quadrotor_simple_force.urdf.xacro"),
    ...
    # Simplified ROS-Gazebo bridge - only essential topics
    gz_ros2_bridge = Node(
        package="ros_gz_bridge",
        executable="parameter_bridge",
        arguments=[
            "/clock@rosgraph_msgs/msg/Clock[gz.msgs.Clock]",
            "/model/quadrotor_0/link/base_link/wrench@geometry_msgs/msg/WrenchStamped[gz.msgs.Wrench]",
            "/quadrotor_0/imu@sensor_msgs/msg/Imu[gz.msgs.IMU]",
        ],
    )

    # CMD_VEL to Wrench converter
    cmd_vel_to_wrench = Node(
        package="falco_drone",
