...
class Controllers:
    def __init__(self, drone_mass=1.0):
        ...
        self.ki_pos = np.array([0.1, 0.1, 0.05])  # Integral gains for x, y, z (reduced Z integral)
        self.kd_pos = np.array([1.0, 1.0, 1.5])  # Derivative gains for x, y, z (reduced Z)

        # PID gains for attitude control - better tuned
        self.kp_att = np.array([3.0, 3.0, 1.5])  # Proportional gains for roll, pitch, yaw
        ...
        # Saturation limits
        self.max_thrust = 25.0  # Maximum thrust (N) - reduced for safety
    ...
    def low_level_control(self, pos_desired, vel_desired, att_desired, pos_current, vel_current, att_current):
        ...
        max_angle = 0.2  # ±11.5 degrees approximately
